...
/*
  connection {
     ...
     private_key = file("~/.ssh/my-terraform-key")             # Path to private key
     host        = self.public_ip
     timeout     = "2m"
   }
*/
...
/*
   provisioner "file" {
     source      = "file10"
     destination = "/home/ubuntu/file10"                       # If we create file that file we call is known as file provisioner
   }
   ...
   provisioner "remote-exec" {
     ...
     ]                                      # the remote-exec means the file or package in install or remote level
}
   
    provisioner "local-exec" {
     command = "touch file500"
  }                                 # it means it store in local where we write the local repo
*/
}
resource "null_resource" "run_script" {             #null resource means we can our owne resoures
  provisioner "remote-exec" {
    connection {
...
#   }
}                                                    # triggers help to idfy the each change in code
...
#terraform taint -we see like this (Apply complete! Resources: 1 added, 0 changed, 1 destroyed.) then before see terraform apply after we like Apply complete! Resources: 1 added, 0 changed, 1 destroyed.